                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-center sticky right-0 z-10 sticky-right">
                      <div className="flex items-center justify-center gap-1">
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-gray-500 hover:text-blue-600" title="Chỉnh sửa" onClick={() => openEdit(user)}>
                          <Pencil className="h-3.5 w-3.5" />
                        </Button>
                        {user.role !== 'admin' && (
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-gray-500 hover:text-red-600" title="Xóa" onClick={() => handleDelete(user.id)}>
