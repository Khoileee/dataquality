...
import {
  ChevronLeft, ChevronRight, Search, Eye, CheckCircle, XCircle,
...
function DqStatusBadge({ tableIds }: { tableIds: string[] }) {
  ...
  // Tooltip content: per-table detail
  ...
  if (noDataCount === total) {
    return (
      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-500 border border-gray-200" title={details}>
        Chưa quét
      </span>
    )
  }
  if (failCount > 0) {
    return (
      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-red-50 text-red-700 border border-red-200" title={details}>
        {failCount}/{total} lỗi
      </span>
    )
  }
  if (warnCount > 0) {
    return (
      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-700 border border-amber-200" title={details}>
        {warnCount}/{total} cảnh báo
      </span>
    )
  }
  return (
    <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200" title={details}>
      {total}/{total} OK
    </span>
  )
}
...
function TableChips({ ids }: { ids: string[] }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-nowrap">
      {visible.map(n => (
        <span key={n} title={n}
          className="px-1.5 py-0.5 rounded text-[11px] font-medium bg-gray-100 text-gray-600 border border-gray-200 max-w-[100px] truncate">
          {n}
        </span>
      ))}
      {overflow > 0 && (
        <div className="relative"
          ...
          <span className="px-1.5 py-0.5 rounded text-[11px] font-semibold bg-blue-100 text-blue-700 border border-blue-200 cursor-default whitespace-nowrap">
            +{overflow}
          </span>
          {open && (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>
                  <span className="inline-flex items-center gap-1">Bảng đầu vào <InfoTooltip text="Các bảng dữ liệu nguồn mà job đọc làm input" /></span>
                </TableHead>
                <TableHead>
                  <span className="inline-flex items-center gap-1">Bảng đầu ra <InfoTooltip text="Các bảng dữ liệu kết quả mà job tạo ra" /></span>
                </TableHead>
                <TableHead className="w-[120px]">
                  <span className="inline-flex items-center gap-1">DQ Input <InfoTooltip text="Trạng thái chất lượng dữ liệu tổng hợp của các bảng đầu vào" /></span>
                </TableHead>
                <TableHead className="w-[120px]">
                  <span className="inline-flex items-center gap-1">DQ Output <InfoTooltip text="Trạng thái chất lượng dữ liệu tổng hợp của các bảng đầu ra" /></span>
                </TableHead>
                <TableHead className="w-[100px]">
                  <span className="inline-flex items-center gap-1">Job Status <InfoTooltip text="Kết quả lần chạy gần nhất của job" /></span>
                </TableHead>
                <TableHead className="w-[120px]">Lịch chạy</TableHead>
                <TableHead className="w-[100px]">Team</TableHead>
                <TableHead className="w-16 text-center sticky right-0 z-10 sticky-right">
                  <span className="inline-flex items-center gap-1">Xem</span>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {paginated.map((job, idx) => (
                <TableRow key={job.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <span className="text-xs text-gray-600 truncate block max-w-[90px]" title={job.team}>{job.team}</span>
                  </TableCell>
                  <TableCell className="sticky right-0 z-10 sticky-right">
                    <div className="flex items-center justify-center">
                      <Button
                        ...
                        onClick={() => {
                          const firstOutput = job.outputTableIds[0]
                          if (firstOutput) navigate(`/data-catalog/${firstOutput}`)
                        }}
                      >
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            <span className="text-xs text-gray-500">
              Hiển thị {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, jobs.length)} / {jobs.length}
            </span>
