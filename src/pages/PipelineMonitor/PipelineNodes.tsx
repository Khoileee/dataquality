...
import {
  Database, FileBarChart, Target, AlertTriangle,
  Cpu, CheckCircle, XCircle, Clock, Loader2
} from 'lucide-react'

// ─── Types ──────────────────────────────────────────────────────────
export type DqGrade = 'pass' | 'warning' | 'fail' | 'no_data'
export type ScanStatus = 'scanned' | 'waiting' | 'scanning' | 'scan_failed' | 'no_schedule'
  ...
  return (
    <div
      className={`
        ...
        hover:shadow-md cursor-pointer min-w-[210px] max-w-[250px]
      `}
    >
      ...
      <div className="px-3 py-2.5 space-y-2">
        ...
        <div className="border-t border-gray-100 pt-1.5 space-y-1">
          <div className={`flex items-center gap-1 text-[11px] font-medium ${scan.color}`}>
            <ScanIcon className={`h-3 w-3 ${data.scanStatus === 'scanning' ? 'animate-spin' : ''}`} />
            <span>{scan.label}</span>
            {data.lastScanned && data.scanStatus === 'scanned' && (
              <span className="text-gray-400 font-normal ml-0.5">lúc {data.lastScanned}</span>
            )}
          </div>
          {data.scanStatus === 'waiting' && data.nextScan && (
            <div className="flex items-center gap-1 text-[10px] text-gray-400">
              <Clock className="h-2.5 w-2.5" />
              <span>Lịch: {data.nextScan}</span>
              {data.scheduleFrequency && <span>· {data.scheduleFrequency}</span>}
            </div>
          )}
        </div>
...
export const NODE_DIMENSIONS = {
  dataSource: { width: 230, height: 160 },
