...
export const mockRuleTemplates: RuleTemplate[] = [
  // Completeness
  { id: 'tmpl-001', name: 'Kiểm tra cột bắt buộc không null', dimension: 'completeness', description: 'Kiểm tra tỷ lệ null của cột bắt buộc phải dưới ngưỡng cho phép', category: 'Null check', usageCount: 145, metricConfig: { metricType: 'not_null', column: '' } },
  { id: 'tmpl-002', name: 'Tổng số cột bắt buộc đầy đủ', dimension: 'completeness', description: 'Bản ghi phải điền đủ tất cả các trường bắt buộc', category: 'Required fields', usageCount: 89, metricConfig: { metricType: 'not_null', column: '' } },
  { id: 'tmpl-003', name: 'Kiểm tra cột có giá trị rỗng', dimension: 'completeness', description: 'Cột chuỗi không được chứa chuỗi rỗng', category: 'Empty string', usageCount: 67, metricConfig: { metricType: 'not_null', column: '' } },
  { id: 'tmpl-004', name: 'Độ phủ dữ liệu tối thiểu', dimension: 'completeness', description: 'Ít nhất X% bản ghi phải có giá trị cho cột này', category: 'Coverage', usageCount: 52, metricConfig: { metricType: 'fill_rate', column: '', minFillPct: 95 } },
  { id: 'tmpl-005', name: 'Kiểm tra dữ liệu theo kỳ', dimension: 'completeness', description: 'Mỗi kỳ báo cáo phải có dữ liệu đầy đủ', category: 'Periodic completeness', usageCount: 34, metricConfig: { metricType: 'fill_rate', column: '', minFillPct: 100 } },
  // Validity
  { id: 'tmpl-006', name: 'Kiểm tra định dạng email', dimension: 'validity', description: 'Email phải đúng định dạng user@domain.com', category: 'Format', usageCount: 98, metricConfig: { metricType: 'format_regex', column: '', pattern: '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$' } },
  { id: 'tmpl-007', name: 'Kiểm tra định dạng số điện thoại VN', dimension: 'validity', description: 'SĐT Việt Nam phải là 10 chữ số bắt đầu bằng 0', category: 'Format', usageCount: 76, metricConfig: { metricType: 'format_regex', column: '', pattern: '^0[0-9]{9}$' } },
  { id: 'tmpl-008', name: 'Giá trị nằm trong danh sách cho phép', dimension: 'validity', description: 'Giá trị cột phải thuộc tập hợp giá trị hợp lệ', category: 'Domain', usageCount: 134, metricConfig: { metricType: 'allowed_values', column: '', allowedValues: ['ACTIVE', 'INACTIVE', 'PENDING'] } },
  ...
  { id: 'tmpl-015', name: 'Kiểm tra tổng kiểm soát', dimension: 'consistency', description: 'Tổng giá trị giữa 2 hệ thống phải khớp nhau', category: 'Cross-system', usageCount: 29, metricConfig: { metricType: 'custom_expression', expression: 'ABS(sum_system_a - sum_system_b) / NULLIF(sum_system_b, 0) * 100 <= 0.1' } },
  // Uniqueness
  { id: 'tmpl-016', name: 'Khóa chính không trùng lặp', dimension: 'uniqueness', description: 'Cột khóa chính không được có giá trị trùng lặp', category: 'Primary key', usageCount: 201, metricConfig: { metricType: 'duplicate_single', column: '' } },
  { id: 'tmpl-017', name: 'Tỷ lệ trùng lặp dưới ngưỡng', dimension: 'uniqueness', description: 'Tỷ lệ giá trị trùng lặp không được vượt quá ngưỡng', category: 'Duplicate rate', usageCount: 78, metricConfig: { metricType: 'duplicate_single', column: '' } },
  { id: 'tmpl-018', name: 'Unique trên tổ hợp cột', dimension: 'uniqueness', description: 'Tổ hợp các cột phải duy nhất', category: 'Composite key', usageCount: 55, metricConfig: { metricType: 'duplicate_composite', columns: [] } },
  // Accuracy
  { id: 'tmpl-019', name: 'Giá trị khớp nguồn tham chiếu', dimension: 'accuracy', description: 'Giá trị cột phải khớp với nguồn dữ liệu tham chiếu', category: 'Reference match', usageCount: 143, metricConfig: { metricType: 'reference_match', sourceColumn: '', refTable: '', refColumn: '' } },
  { id: 'tmpl-020', name: 'Phát hiện giá trị ngoại lệ thống kê', dimension: 'accuracy', description: 'Giá trị không được lệch quá N độ lệch chuẩn', category: 'Statistical outlier', usageCount: 67, metricConfig: { metricType: 'custom_expression', expression: 'COUNT(*) FILTER (WHERE ABS(col - avg_val) > 3 * std_val) / COUNT(*) * 100 <= 1' } },
  // Timeliness
  { id: 'tmpl-021', name: 'Dữ liệu cập nhật trong SLA', dimension: 'timeliness', description: 'Dữ liệu phải được cập nhật trong khoảng thời gian SLA', category: 'SLA', usageCount: 89, metricConfig: { metricType: 'on_time', column: '', slaTime: '08:00', alertWindowMinutes: 30 } },
  { id: 'tmpl-022', name: 'Dữ liệu tươi mới (Freshness)', dimension: 'timeliness', description: 'Timestamp không được cũ quá N giờ so với hiện tại', category: 'Freshness', usageCount: 112, metricConfig: { metricType: 'freshness', column: '', maxAge: 24, maxAgeUnit: 'hours' } },
]
