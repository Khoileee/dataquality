...
2. [Sơ đồ module và luồng dữ liệu](#2-sơ-đồ-module-và-luồng-dữ-liệu)
   - 2.1 [Sơ đồ tổng quan (có STT)](#21-sơ-đồ-tổng-quan-các-module)
   - 2.2 [Luồng thông tin giữa các menu (có STT)](#22-luồng-thông-tin-giữa-các-menu-có-stt)
3. [Thứ tự cấu hình — 8 bước (Setup lần đầu)](#3-thứ-tự-cấu-hình-setup-lần-đầu)
4. [Bảng INPUT → OUTPUT giữa các menu (có STT)](#4-bảng-input--output-giữa-các-menu)
...
5. [Rule DQ: Quy tắc cho Cột hay Bảng?](#5-rule-dq-quy-tắc-cho-cột-hay-bảng)
   - 5B. [Mẫu quy tắc — Hệ thống 3 tầng auto-match](#5b-mẫu-quy-tắc--hệ-thống-3-tầng-auto-match)
...
7. [Hướng dẫn từng menu](#7-hướng-dẫn-từng-menu)
   ...
   - 7.4 [Giám sát Pipeline (`/pipeline`)](#74-giám-sát-pipeline-pipeline) — DQ badge, 7 case hiển thị, SLA
   - 7.5 [Ngưỡng cảnh báo (Thresholds)](#75-ngưỡng-cảnh-báo-thresholds)
   - 7.6 [Quản lý quy tắc (Rules)](#76-quản-lý-quy-tắc-rules) — mẫu quy tắc, template
   - 7.7 [Lịch chạy (Schedules)](#77-lịch-chạy-schedules) — 5 khung giờ, multi-run, batching
   - 7.8 [Vấn đề & Sự cố (Issues)](#78-vấn-đề--sự-cố-issues)
   - 7.9 [Báo cáo (Reports)](#79-báo-cáo-reports)
   - 7.10 [Thông báo (Notifications)](#710-thông-báo-notifications) — 4 mode digest, gom mail, file đính kèm
...
flowchart LR
  ...
  RP --> DB
  DC -->|"chạy quét"| PR
  DC -->|"input/output tables"| PM
  PM -->|"DQ badge"| DB
  IS -->|"trigger"| NT
```

**Ý nghĩa STT trong diagram:**

| STT | Module | Vai trò trong luồng |
|---|---|---|
| ① | Danh mục dữ liệu | Đầu vào — đăng ký bảng cần giám sát |
| ② | Ngưỡng cảnh báo | Định nghĩa "tốt / cảnh báo / không đạt" |
| ③ | Quy tắc | Logic kiểm tra cụ thể (28 metric types) |
| ④ | Lịch chạy | Tự động hóa — chạy rule theo tần suất |
| ⑤ | Vấn đề & Sự cố | Kết quả — tự sinh khi rule fail |
| ⑥ | Báo cáo | Phân tích — xu hướng qua thời gian |
| ⑦ | Dashboard | Tổng quan — KPI + alert |
| — | Phân tích dữ liệu | Hỗ trợ — khám phá dữ liệu trước khi viết rule |
| — | Giám sát Pipeline | Hỗ trợ — theo dõi job ETL + DQ badge |
| — | Thông báo | Hỗ trợ — gửi alert qua email/telegram/webhook |
| — | Cài đặt | Hỗ trợ — template rule, template schedule, quản lý user |

### 2.2 Luồng thông tin giữa các menu (có STT)

```mermaid
flowchart TD
  A["① DataCatalog\nĐăng ký bảng DL"] -->|"❶ bảng ID, tên cột"| B
  A -->|"❷ bảng ID"| D
  A -->|"❸ input/output tables"| J

  B["② Thresholds\nNgưỡng W/C"] -->|"❹ W%, C% theo chiều"| C

  C["③ Rules\nQuy tắc kiểm tra"] -->|"❺ rule gắn bảng"| D

  D["④ Schedules\nLịch chạy tự động"] -->|"❻ kết quả fail"| E
  D -->|"❼ điểm theo thời gian"| F

  E["⑤ Issues\nVấn đề phát sinh"] -->|"❽ số lượng, mức độ"| G
  F["⑥ Reports\nXu hướng chất lượng"] -->|"❾ tổng hợp"| G

  G["⑦ Dashboard\nTổng quan KPI"]

  E -->|"❿ trigger event"| H["Notifications\nGửi email/telegram/webhook"]

  A -->|"chạy quét thủ công"| I["Profiling\nThống kê cột (snapshot)"]

  J["Pipeline Monitor\nGiám sát job ETL"] -->|"DQ badge"| G
  H -->|"gửi kèm file"| H2["Email / Telegram\n(CSV sample, PDF report)"]
```

**Giải thích luồng theo STT:**

| STT | Từ → Đến | Dữ liệu truyền | Ý nghĩa |
|---|---|---|---|
| ❶ | DataCatalog → Thresholds | Bảng ID, tên cột | Để đặt ngưỡng W/C cho từng bảng/chiều |
| ❷ | DataCatalog → Schedules | Bảng ID | Để chọn bảng khi tạo lịch chạy |
| ❸ | DataCatalog → Pipeline Monitor | inputTableIds, outputTableIds | Liên kết job ETL với bảng DQ |
| ❹ | Thresholds → Rules | W%, C% theo chiều | Pre-fill ngưỡng mặc định khi tạo rule mới |
| ❺ | Rules → Schedules | Rule gắn với bảng | Lịch chạy biết cần thực thi rule nào |
| ❻ | Schedules → Issues | Kết quả fail/warning | Tự động sinh Issue khi rule không đạt |
| ❼ | Schedules → Reports | Điểm qua thời gian | Dữ liệu cho biểu đồ xu hướng |
| ❽ | Issues → Dashboard | Số lượng, mức độ | KPI cards: issue mới, critical, resolved |
| ❾ | Reports → Dashboard | Điểm tổng hợp | Trend chart, radar 6 chiều |
| ❿ | Issues → Notifications | Trigger event | Gửi mail/telegram khi phát sinh vấn đề |
...
flowchart LR
  ...
  style S7 fill:#f0fdf4,stroke:#16a34a
  style S8 fill:#fff7ed,stroke:#ea580c
```
...
| ③ | Quy tắc | `/rules` | Tạo rule kiểm tra cụ thể: chọn bảng → chọn chiều → chọn metric type → điền tham số → xác nhận ngưỡng W/C. Có thể dùng **mẫu quy tắc** để tạo nhanh (xem [section 5B](#5b-mẫu-quy-tắc--hệ-thống-3-tầng-auto-match)) | Xác định chính xác **cái gì** và **cách nào** cần kiểm tra cho từng bảng | Đã có bảng ở bước ①. Ngưỡng ở bước ② (nếu có) sẽ pre-fill | Không có rule → Schedule chạy nhưng không kiểm tra gì → không sinh điểm |
| ④ | Lịch chạy | `/schedules` | Cấu hình lịch tự động: chọn bảng, tần suất (hàng giờ/ngày/tuần/tháng), giờ chạy. Xem [ý nghĩa 5 khung giờ](#ý-nghĩa-5-khung-giờ-chạy) để chọn giờ phù hợp | Tự động hóa kiểm tra — mỗi lần lịch chạy sẽ thực thi **toàn bộ Rule active** của bảng đó | Đã có bảng ở bước ① và ít nhất 1 Rule active ở bước ③ | Rule chỉ chạy khi user nhấn "Chạy ngay" thủ công — không tự động |
...
| ⑥ | Báo cáo | `/reports` | Xem xu hướng điểm chất lượng theo thời gian, so sánh giữa các bảng, xuất báo cáo. Tab Reconciliation để đối soát số liệu | Đánh giá chất lượng dài hạn — phát hiện xu hướng xấu đi hoặc cải thiện | Cần có lịch sử chạy Rule qua nhiều ngày | Không có dữ liệu xu hướng — khó đánh giá được tiến bộ |
| ⑦ | Giám sát Pipeline | `/pipeline` | Đăng ký job ETL (Spark/Pentaho/Airflow): tên job, bảng input/output, lịch chạy, owner. DQ badge tự tính từ bảng liên quan | Theo dõi trạng thái job ETL + chất lượng DQ liên kết — phát hiện job lỗi hoặc SLA miss | Đã có bảng ở bước ① (để chọn input/output tables) | Không giám sát được pipeline, mất liên kết giữa job ETL và DQ |
| ⑧ | Thông báo | `/notifications` | Cấu hình gửi alert: chọn channel (email/telegram/webhook), trigger (warning/critical/resolved), người nhận, digest mode. Xem [4 mode gửi](#4-mode-gửi-thông-báo-nghiệp-vụ) | Đảm bảo đúng người nhận đúng alert đúng lúc — tránh spam, không bỏ sót critical | Cần có Issues sinh ra (bước ⑤) để trigger | Mọi alert phải check thủ công trên Dashboard — dễ bỏ sót |

> **Lưu ý:**
> - Bước ② (Ngưỡng) có thể làm sau Bước ③ vì hệ thống có giá trị mặc định. Tuy nhiên khuyến nghị làm trước để tiết kiệm thời gian chỉnh sửa W/C cho từng Rule.
> - Bước ⑦ (Pipeline) và ⑧ (Notifications) có thể thực hiện **song song** với bước ①–⑥. Pipeline Monitor cần bảng đã đăng ký, Notifications cần Issues đã sinh ra.
> - Ngoài 8 bước trên, có thể chạy **Profiling** (quét thống kê) bất kỳ lúc nào sau bước ① để khám phá dữ liệu trước khi viết rule.
...
| STT | Menu | URL | Bạn nhập/cấu hình | Tạo ra (Output) | Được dùng ở menu | Xem kết quả ở |
|---|---|---|---|---|---|---|
| ① | **Danh mục dữ liệu** | `/data-catalog` | Tên bảng, schema, loại kết nối, chủ sở hữu, `dataRequiredByTime` (giờ cần dữ liệu) | Danh sách bảng DL có ID | Rules, Schedules, Thresholds, Profiling, Pipeline Monitor | DataCatalog > chi tiết bảng |
| — | **Phân tích dữ liệu** | `/profiling` | Chọn bảng → nhấn "Chạy phân tích mới" | Snapshot thống kê cột (null%, distinct%, min/max, kiểu dữ liệu) | DataCatalog > tab "Phân tích cột", Rules (gợi ý metric) | Profiling > Xem chi tiết |
| ② | **Ngưỡng cảnh báo** | `/thresholds` | W%, C% cho 6 chiều x toàn cục + riêng bảng | Ngưỡng mặc định để pre-fill khi tạo Rule | Rules (pre-fill W/C) | Thresholds > thanh màu trực quan |
| ③ | **Quy tắc** | `/rules` | Chọn bảng, chiều, metric type (28 loại), tham số, W/C. Có thể dùng **mẫu quy tắc** auto-match | Rule gắn với bảng, chứa logic kiểm tra | Schedules (gắn rule vào lịch) | Rules > cột "Kết quả", "Điểm" |
| ④ | **Lịch chạy** | `/schedules` | Chọn bảng, tần suất, giờ chạy (xem 5 khung giờ). Workaround tạo 2 schedule cho bảng quét 2 lần/ngày | Lịch tự động chạy toàn bộ rule của bảng | Issues (tự sinh), Reports, Dashboard | Schedules > "Xem Issues" |
| ⑤ | **Vấn đề & Sự cố** | `/issues` | *(Tự sinh từ Schedules)* — Gán người, đổi trạng thái, ghi chú timeline | Vấn đề được xử lý, downstream impact | Dashboard, Reports, Notifications | Issues > chi tiết > timeline |
| ⑥ | **Báo cáo** | `/reports` | Chọn khoảng thời gian, bảng, chiều. Tab Reconciliation để đối soát | Biểu đồ xu hướng, radar chart, variance report | Dashboard | Reports > Overview / Reconciliation |
| ⑦ | **Giám sát Pipeline** | `/pipeline` | Tên job ETL, bảng input/output, lịch chạy, owner, nhóm | Job metadata + DQ badge (tính từ `overallScore` bảng liên quan) | Dashboard (DQ badge) | Pipeline Monitor > danh sách job |
| ⑧ | **Thông báo** | `/notifications` | Channel (email/telegram/webhook), trigger (warning/critical/resolved), người nhận, digest mode (immediate/batched/daily/weekly), file đính kèm | Thông báo tự động gửi đi + file CSV/PDF/XLSX | *(nhận qua email/telegram/webhook)* | Notifications > lịch sử gửi |
| — | **Cài đặt** | `/settings` | Template rule, template schedule, quản lý user/role | Mẫu tái sử dụng cho bước ③ ④ | Rules, Schedules | Settings > 3 tab |
...
### 5B. Mẫu quy tắc — Hệ thống 3 tầng auto-match

Hệ thống sử dụng kiến trúc **3 tầng mẫu quy tắc** (Template Hierarchy) để giảm công sức khi tạo rule cho bảng mới. Khi đăng ký bảng mới vào Danh mục dữ liệu, hệ thống tự động gợi ý rule dựa trên kiểu cột và pattern đã học từ các mẫu.

#### Sơ đồ 3 tầng

```
Tầng 1 — Metric Template (chung cho toàn hệ thống)
    │  Ví dụ: "Null rate check", "Duplicate PK check"
    │  Áp dụng: mọi bảng, mọi cột phù hợp
    │
    ├──▶ Tầng 2 — Column Profile (cho 1 cột cụ thể)
    │       Ví dụ: "customer_id null rate < 0.1%"
    │       Áp dụng: chỉ cột customer_id trong bảng cụ thể
    │
    └──▶ Tầng 3 — Table Profile (cho 1 bảng)
            Ví dụ: "GD_GIAODICH row_count > 1000 mỗi ngày"
            Áp dụng: toàn bảng, không gắn cột
```

#### Phân biệt 3 tầng

| Tầng | Phạm vi | Khi nào dùng | Ví dụ | Nơi quản lý |
|------|---------|--------------|-------|-------------|
| **Metric Template** | Generic, mọi bảng | Quy tắc chung toàn công ty | "Cột kiểu VARCHAR: null < 1%, distinct > 10" | Settings > Quy tắc mặc định |
| **Column Profile** | 1 cột cụ thể | Quy tắc đặc thù nghiệp vụ của cột | "SO_CMND: regex 9/12 số, unique 100%" | Rules > Tạo mới > chọn cột |
| **Table Profile** | 1 bảng | Quy tắc mức bảng (row_count, timeliness...) | "Bảng GD: row_count 1K-5M, load trước 08:00" | Rules > Tạo mới > chọn bảng |

#### Luồng auto-match khi thêm bảng mới

```
(1) User thêm bảng "KH_KHACHHANG" vào Danh mục dữ liệu
     │
     ▼
(2) Hệ thống đọc danh sách cột + kiểu dữ liệu
     │  Ví dụ: MA_KH (varchar), HO_TEN (varchar), EMAIL (varchar),
     │          NGAY_SINH (date), SO_DU (decimal)
     │
     ▼
(3) Auto-match từ Metric Template:
     │  • VARCHAR → suggest: null_rate, distinct_count, max_length
     │  • DATE    → suggest: null_rate, value_range (min/max)
     │  • DECIMAL → suggest: null_rate, value_range, statistical_outlier
     │
     ▼
(4) Hiển thị gợi ý trên giao diện Rules → user chọn áp dụng hoặc bỏ qua
```

> **Lưu ý:** Auto-match dựa trên kiểu dữ liệu của cột. User có thể tùy chỉnh ngưỡng warning/critical sau khi áp dụng. Mẫu quy tắc được quản lý tại menu **Settings > Quy tắc mặc định**.
...
### 7.4 Giám sát Pipeline (`/pipeline`)

**URL:** `/pipeline` | **Mục đích:** Giám sát toàn bộ job xử lý dữ liệu (ETL/Spark/Pentaho), theo dõi bảng đầu vào/đầu ra, trạng thái DQ và kết quả chạy theo thời gian thực.

#### Luồng thực tế trong tổ chức

```
Dữ liệu phát sinh → Data Engineer ingest về Data Lake
  → Job xử lý (Spark/SQL/Pentaho) → Lập lịch trên Rundeck/Airflow
  → Trigger tự động → Ghi kết quả vào DB/Data Mart
  → DQ Tool quét rule → Hiển thị kết quả trên Pipeline Monitor
  → BI Dashboard / Báo cáo
```

#### Danh sách jobs — Các cột hiển thị

| Cột | Ý nghĩa |
|---|---|
| Tên Job | Tên định danh của job ETL |
| Mô tả | Mô tả ngắn về mục đích job |
| Bảng đầu vào | Các bảng job đọc data từ đó (input) |
| Bảng đầu ra | Các bảng job ghi kết quả vào (output) |
| DQ | Badge tổng hợp chất lượng dữ liệu của tất cả bảng input + output |
| Tiến độ quét | Thanh tiến trình quét rule cho bảng liên quan |
| Trạng thái | Badge hiển thị kết quả lần chạy gần nhất |
| Lịch chạy | Tần suất chạy (VD: "Hàng ngày 06:00") |

#### Trạng thái job (cột "Trạng thái")

Hiển thị dưới dạng Badge có màu, chữ không xuống dòng:

| Trạng thái | Badge | Ý nghĩa | Khi nào xảy ra |
|---|---|---|---|
| **Thành công** | Badge xanh (success) | Job hoàn thành, dữ liệu load đầy đủ | Orchestrator báo exit code 0, output table pass validation |
| **Lỗi** | Badge đỏ (destructive) | Job bị lỗi | Exception/timeout; hoặc input table có DQ issue critical khiến transform abort |
| **Một phần** | Badge vàng (warning) | Chạy xong nhưng chỉ load 1 phần | Timeout giữa chừng, partial data committed |
| **Đang chạy** | Badge xanh dương + spinner | Job đang thực thi | Orchestrator chưa báo kết quả |

#### Cột DQ — Badge chất lượng dữ liệu

Cột DQ tổng hợp kết quả quét rule từ **tất cả bảng input + output** của job:

| DQ Badge | Ý nghĩa | Điều kiện |
|---|---|---|
| Badge xanh (success): `N/N OK` | Tất cả bảng pass | Mọi bảng liên quan có `overallScore` >= warning threshold |
| Badge vàng (warning): `X/N cảnh báo` | Có bảng cảnh báo | Ít nhất 1 bảng có warning, không có critical |
| Badge đỏ (destructive): `X/N lỗi` | Có bảng fail | Ít nhất 1 bảng có critical issue |
| Badge xám (secondary): `Chưa quét` | Chưa có dữ liệu DQ | Bảng chưa được quét rule hoặc profiling |

> **Cách tính DQ:** DQ badge KHÔNG lưu trực tiếp trên PipelineJob. Hệ thống tính real-time bằng cách lookup `DataSource.overallScore` + `lastProfiled` qua `inputTableIds[]` + `outputTableIds[]`.

#### 7 case hiển thị trên Pipeline Monitor

| # | Case | Hiển thị | Hành vi |
|---|------|---------|---------|
| 1 | Job done + DQ pass | Badge Trạng thái xanh + Badge DQ xanh | Bình thường |
| 2 | Job done + DQ warning | Badge Trạng thái xanh + Badge DQ vàng | Nhấn DQ badge → xem Issues |
| 3 | Job done + DQ fail | Badge Trạng thái xanh + Badge DQ đỏ | Nhấn DQ badge → xem Issues critical |
| 4 | Job chạy lâu quá SLA | Badge "Quá hạn SLA 08:00" đỏ | Gửi notification ngay ⚠️ (TO-BE) |
| 5 | Job chưa quét DQ | Badge DQ xám "Chưa quét" | Gợi ý chạy profiling/tạo rule |
| 6 | Job đang chạy | Badge xanh dương + spinner | Không cho phép chạy lại khi đang running |
| 7 | Job quét 2 lần/ngày | Hiển thị lần quét gần nhất + tooltip lịch sử | Xem chi tiết để thấy tất cả lần chạy |

#### Cơ chế liên kết Job ↔ DQ

```
PipelineJob
  ├── inputTableIds[]  ──▶ DataSource (tra overallScore + lastProfiled)
  ├── outputTableIds[] ──▶ DataSource (tra overallScore + lastProfiled)
  └── schedule: string    (free-text, VD: "Hàng ngày 06:00")
                           ⚠️ Chưa link đến Schedule entity
```

- Liên kết qua **tableId** (không phải jobName)
...
#### Hiển thị SLA (giờ cần dữ liệu)

> ⚠️ **Đề xuất TO-BE** — field `DataSource.dataRequiredByTime` đã khai báo trong data model nhưng chưa có logic UI sử dụng.

**Kỳ vọng:**
- Bảng có `dataRequiredByTime` (VD: `08:00`) → hiển thị cột "SLA" trên Pipeline Monitor
- Nếu `now() > SLA` mà job chưa hoàn thành → hiển thị cảnh báo đỏ "Quá hạn SLA"
- Auto-trigger notification ngay khi phát hiện SLA miss

**Ví dụ hiển thị kỳ vọng:**

```
┌──────────────┬────────┬────────┬─────────┬──────┬───────────────┐
│ Tên Job      │ Input  │ Output │ DQ      │ SLA  │ Trạng thái    │
├──────────────┼────────┼────────┼─────────┼──────┼───────────────┤
│ ETL_EOD      │ tx_raw │ tx_dim │ 92 Pass │ 08:00│ Thành công    │
│ ETL_FX_06H   │ fx_src │ fx_dim │ 45 Fail │ 06:00│ Quá hạn SLA   │
│ ETL_CUST     │ cust_s │ cust_d │ Chưa quét│ —   │ Đang chạy...  │
└──────────────┴────────┴────────┴─────────┴──────┴───────────────┘
```
...
#### Dùng mẫu quy tắc (template) để tạo nhanh

Thay vì nhập thủ công toàn bộ cấu hình mỗi lần tạo rule, có thể dùng **mẫu quy tắc** (xem chi tiết tại [section 5B](#5b-mẫu-quy-tắc--hệ-thống-3-tầng-auto-match)):

1. Vào **Rules** → tab **"Templates"** (nếu có) hoặc nhấn **"Tạo từ mẫu"**
2. Hệ thống hiển thị danh sách Metric Template đã định nghĩa sẵn
3. Chọn template → form tạo rule **auto pre-fill** các field: metric type, dimension, ngưỡng mặc định
4. User chỉ cần chọn bảng/cột áp dụng và tùy chỉnh ngưỡng nếu cần

> **Quản lý mẫu:** Vào menu **Settings > Quy tắc mặc định** (`/settings/default-rules`) để tạo/sửa/xóa template. Template được tái sử dụng khi tạo rule cho bất kỳ bảng nào.
...
#### Ý nghĩa 5 khung giờ chạy

Hệ thống chia lịch chạy thành 5 khung giờ + Realtime (xem ở chế độ hiển thị "Theo khung giờ" trên giao diện Schedules). Mỗi khung giờ phục vụ mục đích nghiệp vụ khác nhau:

| Khung giờ | Đặc điểm dữ liệu | Loại rule thường chạy |
|---|---|---|
| **00-06** (Rạng sáng) | EOD batch: Pentaho nạp L1 → L2 xong | Completeness, Uniqueness — kiểm tra dữ liệu đã nạp đầy đủ |
| **06-10** (Sáng) | Dashboard cần sẵn trước 8h — SLA quan trọng | Timeliness — kiểm tra SLA 08:00 |
| **10-14** (Trưa) | Intraday update, dữ liệu nóng | Freshness, Consistency — kiểm tra tính mới và nhất quán |
| **14-18** (Chiều) | Báo cáo quản lý | Accuracy, Reconciliation — đối soát chính xác |
| **18-24** (Tối) | Pre-EOD — chuẩn bị đóng sổ | Cross-table validation, tổng hợp |
| **Realtime** | Luôn chạy — stream/hourly | Validity — kiểm tra từng giao dịch |

> **Lưu ý:** Phân bổ khung giờ hiện tại là view-only (hiển thị nhóm lịch theo giờ chạy). Chưa có cơ chế tự động phân bổ rule vào khung giờ — user tự chọn `runTime` khi tạo Schedule.

#### Case đồng bộ 2 lần/ngày

Một số bảng cần đồng bộ nhiều lần trong ngày. Ví dụ:

| Bảng | Lần 1 | Lần 2 | Lần 3+ |
|---|---|---|---|
| `tbl_transaction` | 06:00 (intraday snapshot) | 23:30 (EOD final) | — |
| `tbl_fx_rate` | 00:00 | 06:00 | 12:00, 18:00 |
| `tbl_customer` | 02:00 | — | — |

**Giới hạn hiện tại:**
- `Schedule.runTime` chỉ là **1 string `HH:mm` duy nhất**, không hỗ trợ mảng nhiều giờ.
- `ScheduleFrequency` không có option `twice_daily` hay `multiple_times_per_day`.

**Cách xử lý (workaround):** Tạo **2 Schedule riêng** cùng rule nhưng khác `runTime`:
- Schedule "Kiểm tra GD_06H" → `runTime = 06:00`
- Schedule "Kiểm tra GD_EOD" → `runTime = 23:30`

> ⚠️ **Đề xuất TO-BE:** Hỗ trợ `runTimes: string[]` hoặc cho phép nhiều Schedule cùng ruleId (option đề xuất: tạo nhiều Schedule, mỗi lần chạy = 1 Rundeck jobId riêng — không cần thay đổi schema).

#### Case quét 2 lần — Pipeline reset hay append?

Khi cùng 1 bảng được quét DQ 2 lần trong 1 ngày (lần 06:00 và lần 23:30):

**Hiện trạng:** `DataSource.overallScore` chỉ giữ **1 giá trị** — lần quét sau **ghi đè** lần trước. Không có entity lưu lịch sử từng lần chạy trong ngày.

**Các case cần quy tắc xử lý:**

| # | Case | Hành vi mong muốn |
|---|------|-------------------|
| 1 | Lần 1 (06:00) success → lần 2 (23:30) chạy | Pipeline Monitor giữ badge lần 1, thêm record lần 2 |
| 2 | Lần 1 success → lần 2 fail | Badge tổng hợp: "1/2 lỗi" (tổng hợp trong ngày) |
| 3 | Lần 1 fail → lần 2 success (retry) | Issue lần 1 auto-resolve khi lần 2 pass? ⚠️ Cần quyết định |
| 4 | Lần 2 đang chạy khi lần 1 chưa xong | Block hoặc kill lần 1? ⚠️ Cần quy tắc |
| 5 | Cron bị skip 1 ngày (không chạy) | Hiển thị "No run today" + sinh Issue timeliness? |

> ⚠️ **Đề xuất TO-BE:** Thêm entity `PipelineJobRun` (lưu history từng lần chạy: `jobId`, `runDate`, `runSequence`, `startedAt`, `finishedAt`, `status`, `dqScore`, `triggerSource`). Hiển thị "latest" + cho phép xem list tất cả lần chạy trong ngày.

#### Gom nhóm lịch (batching)

> ⚠️ **Đề xuất TO-BE** — hiện chưa có concept batching trong data model.

**Bài toán:** Tránh tình trạng 100 bảng cùng quét lúc 00:00 khiến cluster Spark/HDFS nghẽn. Cần gom nhóm theo ưu tiên và dependency.

**Field đề xuất thêm vào Schedule:**

| Field | Kiểu | Mục đích |
|---|---|---|
| `priority` | `P1` / `P2` / `P3` | Phân biệt rule critical để ưu tiên chạy trước |
| `batchGroup` | string | Gom schedule cùng nhóm (VD: `batch-payment-EOD`) |
| `maxParallelism` | number | Giới hạn số rule chạy đồng thời trong nhóm |
| `dependsOn` | scheduleId[] | Schedule B chỉ chạy sau khi dependsOn A success |

**Ví dụ batching:**
```
Batch "payment-EOD" (priority P1, maxParallelism 3):
  ├── Schedule "GD_GIAODICH" (06:00)     ← chạy trước
  ├── Schedule "KH_KHACHHANG" (06:15)    ← chạy sau GD
  └── Schedule "BAO_CAO_NGAY" (07:00)    ← dependsOn GD + KH
```
...
#### Chế độ gửi tổng hợp (Digest)

Khi tạo/sửa cấu hình thông báo, có toggle **"Gửi tổng hợp (digest)"**:
- **Tắt** (mặc định): mỗi sự kiện gửi 1 thông báo riêng (mode Immediate)
- **Bật**: gom nhiều sự kiện lại gửi 1 lần theo interval (nhập số phút, mặc định 15)

> **Lưu ý:** Hiện tại UI cho phép cấu hình digest, nhưng engine runtime gom thông báo chưa implement (mock). Trong production cần xây queue/buffer để gom.

#### 4 mode gửi thông báo (nghiệp vụ)

| Mode | Khi nào dùng | Delay | Nội dung gửi |
|---|---|---|---|
| **Immediate** (gửi ngay) | Rule CRITICAL fail, SLA miss | < 1 phút | 1 mail/telegram cho 1 sự kiện |
| **Batched** (gom theo interval) | Rule WARNING | 15/30/60 phút (tùy config) | 1 mail cho N sự kiện, gom theo bảng → rule |
| **Daily Digest** (tổng hợp cuối ngày) | Báo cáo tổng quan + resolved | 1 lần/ngày (VD: 18:00) | Mail HTML + đính kèm Excel chi tiết |
| **Weekly Digest** (trend) | Cấp quản lý | Thứ 2 hàng tuần 08:00 | Mail + PDF dashboard snapshot |

#### Logic gom thông báo khi nhiều rule fail

Khi nhiều rule fail cùng lúc, hệ thống KHÔNG gửi N mail riêng rẽ mà gom lại:

| Tình huống | Cách gom |
|---|---|
| 5 rule **cùng 1 bảng** fail cùng lúc | Gom thành **1 mail**: "Bảng X có 5 rule lỗi" |
| 20 rule **rải rác nhiều bảng** fail | Gom **theo owner** — mỗi owner nhận 1 mail tổng hợp |
| Rule WARNING (không critical) | Gom **theo interval** (VD: 15 phút gom 1 lần) |
| Rule resolved (tự pass lại) | Gom vào **daily digest** — thông báo kết quả cuối ngày |
| Pipeline SLA miss | Gửi **immediate** + tag @owner trên Telegram |

**Ví dụ email gom theo bảng:**

```
Subject: [DQ CRITICAL] Bảng tbl_transaction có 5 rule lỗi — 2026-04-16 06:15

Bảng: tbl_transaction (Owner: phongdh@company.com)
SLA giờ cần dữ liệu: 08:00
Thời gian phát hiện: 06:15

Danh sách vi phạm:
+-----------------------+-------------+--------+--------------+
| Rule                  | Dimension   | Score  | Nguong       |
+-----------------------+-------------+--------+--------------+
| Null customer_id      | Completeness| 45     | Critical <70 |
| Duplicate txn_id      | Uniqueness  | 62     | Critical <70 |
| Invalid amount format | Validity    | 58     | Critical <70 |
| Amount > 1B suspect   | Accuracy    | 40     | Critical <70 |
| Timeliness miss       | Timeliness  | 0      | Critical <70 |
+-----------------------+-------------+--------+--------------+

Chi tiết: [Mo tren DQ Tool]
Dinh kem: sample_violations_20260416.csv (top 100 dong vi pham)
```

#### Ma trận quyết định: mode x channel x attachment

| Điều kiện | Mode gửi | Channel | File đính kèm |
|---|---|---|---|
| CRITICAL + bảng có SLA | Immediate | Email + Telegram | CSV sample vi phạm |
| CRITICAL + bảng không SLA | Immediate | Email | CSV sample vi phạm |
| WARNING | Batched 15' | Email | Không |
| RESOLVED | Daily digest | Email | Không |
| Pipeline SLA miss | Immediate | Telegram | Không |
| Weekly summary | Weekly digest | Email | PDF dashboard |

#### File đính kèm (attachment)

> ⚠️ Hiện tại `NotificationConfig` **chưa có** logic attachment file — đang mock. Cần bàn với đội hạ tầng.

**5 loại file đề xuất:**

| # | Loại file | Nguồn sinh | Format | Khi nào gửi |
|---|---|---|---|---|
| 1 | **Sample vi phạm** | Rule run output (top 100 dòng) | CSV | Khi CRITICAL issue |
| 2 | **Báo cáo chi tiết rule run** | Tổng hợp từ RuleRunResult | PDF/XLSX | Daily digest |
| 3 | **Profiling summary** | Từ ProfilingResult | XLSX | Khi chạy profiling xong |
| 4 | **Reconciliation variance** | Màn Reports Reconciliation | XLSX | Khi variance > ngưỡng |
| 5 | **Dashboard snapshot** | Export dashboard | PDF | Weekly digest (thứ 2) |

**Câu hỏi cần xác nhận với hạ tầng:**
- Vị trí lưu file: HDFS / Object Storage / Local filesystem?
- Retention: giữ file bao lâu (30 ngày / 90 ngày)?
- Giới hạn kích thước: Telegram ~50MB, Email SMTP tùy chính sách
- Quyền truy cập: pre-signed URL hay đính kèm trực tiếp?

#### Luồng gửi notification tổng thể

```
Rule Run FAILED / WARNING
    |
    v
Sinh Issue --> check NotificationConfig match (trigger, scope, tables)
    |
    +---> digest mode?
    |       |
    |       +-- OFF --> gửi ngay (Immediate)
    |       |
    |       +-- ON  --> đẩy vào queue --> gom theo interval --> gửi batch
    |
    +---> Channel = email
    |       +-- Render template --> body HTML
    |       +-- Gen attachment (CSV sample) --> lưu tạm
    |       +-- SMTP send --> ghi history
    |
    +---> Channel = telegram
    |       +-- Render template --> markdown
    |       +-- sendMessage (Bot API)
    |       +-- sendDocument (nếu có attachment)
    |
    +---> Channel = webhook
            +-- POST JSON payload chuẩn
```
